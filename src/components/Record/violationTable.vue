...
import { ref, computed, onBeforeUnmount, watch, onBeforeMount, nextTick } from 'vue'
import { storeToRefs } from 'pinia';
import { useRecordStore } from '@/stores'
import type { RecordVd, SortRule, SelectList, Column } from '@/composables/models'
import { SelectListClass } from "@/composables/models";
import { useI18n } from 'vue-i18n';

const { t } = useI18n();
const { getRecordByPageApi, getRecordTotalPageApi, turnOnEventScreenshotModal, turnOnEventVideoModal, resetCurrentPage } = useRecordStore()
const { recordVdList, filterDropDownList, selectList, isHideStreamFilter, pageSize, totalPage, currentPage, filterValue, resetTiming, conditionForVdApi: conditionForRecordApi, isDataLoaded, isPageNoLoaded, sortDirection, channelNameFilter, eventNameFilter, eventTypeShowed } = storeToRefs(useRecordStore())
const headerList = ref<SortDirection[]>([{ name: t('record.Time'), apiLabel: 'Time' }, { name: t('record.Stream_name'), apiLabel: 'ChannelName' }, { name: t('record.Vehicle_type'), apiLabel: 'VehicleType' }, { name: t('record.Event_name'), apiLabel: 'EventName' }, { name: t('record.Event_type'), apiLabel: 'EventType' }, { name: t('record.Road_name'), apiLabel: 'RoadName' }, { name: t('record.License_plate'), apiLabel: 'LicensePlate' }])
const headerWithDroplist = ref<string[]>([t("record.Stream_name"), t("record.Event_name"), t("record.Event_type")])
const sortDirectionList = ref<string[]>(['none', 'none', 'none', 'none', 'none', 'none', 'none', 'none', 'none', 'none', 'none'])
const isDropdownFilterUsed = ref<boolean>(false)
...
const searchChannelInput = ref<string>('')
const searchEventInput = ref<string>('')
...
const isEventTypeShowed = ref<boolean[]>(Array.from({ length: eventTypeShowed.value.length }, () => true))
...
const blankRowShow = computed(() => {
    let number = 0
    if (recordVdList.value && recordVdList.value.length < 16) number = 16 - recordVdList.value.length
    return number
})
...
const selectAllData = (_bool: boolean) => {
    let list: RecordVd[] = [...recordVdList.value]
    if (!_bool) {
...
const selectData = (_bool: boolean, data: RecordVd, id: string) => {
    _bool = !_bool
...
const initialSelectMatrix = (columns: number = recordVdList.value.length): SelectList[] => {
    const newMatrix: SelectList[] = [];
    for (let i = 0; i < columns; i++) {
        const item: SelectList = {
            id: recordVdList.value[i].recordId
        }
        newMatrix.push(item)
    }
...
const handleResetAll = () => {
    ...
    isEventTypeShowed.value = Array.from({ length: eventTypeShowed.value.length }, () => true)
...
const changeEventTypeShowed = async () => {
    eventTypeShowed.value.forEach((item: Column, index) => {
        item.visible = isEventTypeShowed.value[index]
    })
    await getRecordByPageApi(currentPage.value)
    await getRecordTotalPageApi()
}
...
watch(recordVdList, () => {
    if (recordVdList.value.length === 0) return
...
<template>
    <div v-if="recordVdList.length !== 0 && isDataLoaded && isPageNoLoaded" class="tableWrapper scrollbar">
        ...
        <table class="tableContent">
                ...
                <tr>
                    ...
                    <th v-for="(label, index) in  headerList " :key="label.name">
                        <div
                            ...
                            <ul v-if="label.name === t('record.Event_type')" class="nameFilter__menu eventType">
                                <li v-for="(value, key) in eventTypeShowed" :class="{ 'checked': isEventTypeShowed[key] }"
                                    class="nameFilter__menu__item btn">
                                    <label class="nameFilter__menu__item__checkbox">
                                        <input v-model="isEventTypeShowed[key]" type="checkbox">
                                        <div></div>
                                    </label>
                                    <div class="nameFilter__menu__item__text">{{ value.label }}</div>
                                </li>
                                <div class="nameFilter__menu__apply">
                                    <button @click="changeEventTypeShowed" class="btn">{{ $t('record.Apply') }}</button>
                                </div>
                            </ul>
                    ...
                    <th>
                        <div>
                            <VTooltip :disabled="isDisableTooltip">
                                <span @mouseover="toggleTooltip">{{ $t('record.Video_link') }}</span>
                                <template #popper>
                                    {{ $t('record.Video_link') }}
                                </template>
                            </VTooltip>
            ...
            <tbody class="tableBody">
                <tr v-for="( data, index ) in  recordVdList " :key="data.recordId">
                    <th class="selector">
                        <label>
                    ...
                    <td>
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ data.vehicleType }}</div>
                            <template #popper>
                                {{ data.vehicleType }}
                            </template>
                        </VTooltip>
                    ...
                    <td>
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ data.eventType }}</div>
                            <template #popper>
                                {{ data.eventType }}
                            </template>
                        </VTooltip>
                    ...
                    <td>
                        <img alt="img failed" class="tableBody__screenImg" 
                            @click="turnOnEventVideoModal(data.recordId, data.licensePlate)"
                            src="\play_video.svg">
                    </td>
                </tr>
    ...
    <div v-else-if="recordVdList.length === 0 && isDropdownFilterUsed && isDataLoaded && isPageNoLoaded">
        <table class="tableContent">
            <thead class="tableHeader">
                <tr>
                    <th class="headerSelector">
    ...
    <div v-else-if="recordVdList.length === 0 && filterValue && isDataLoaded && isPageNoLoaded"
    ...
    <div v-else-if="recordVdList.length === 0 && isDataLoaded && isPageNoLoaded" class="tableWrapper noResult">
