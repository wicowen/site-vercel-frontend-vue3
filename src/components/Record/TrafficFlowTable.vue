...
import type { RecordTFA, SortRule, SelectList } from '@/composables/models'
...
const headerList = ref<SortDirection[]>(
    [
        { name: t('record.Time'), apiLabel: 'StartTime' },
        ...
        { name: t('record.Truck'), apiLabel: 'Truck' },
        { name: t('record.Bus'), apiLabel: 'Bus' },
        { name: t('record.Car'), apiLabel: 'Car' },
        { name: t('record.Motorbike'), apiLabel: 'Motorbike' },
        { name: t('record.Bike'), apiLabel: 'Bike' },
        { name: t('record.Person'), apiLabel: 'Person' },
        { name: t('record.Total'), apiLabel: 'Total' }
    ]
)
...
const tableSelectList = ref<SelectList[]>(Array.from({ length: pageSize.value }, () => new SelectListClass()))
...
const isTableSelectAll = computed(() => {
    if (!tableSelectList.value) return false
...
const blankRowShow = computed(() => {
    let number = 0
    if (recordTfaList.value && recordTfaList.value.length < 16) number = 16 - recordTfaList.value.length
    return number
})

const isTotalShow = computed(() => {
    let _bool = vehicleTypeShowed.value.some(i => i.visible === true)
    return _bool
})
...
// function sortDataAsce(column: keyof RecordTFA, data: RecordTFA[]) {
//     const sorted = [...data]
//     return sorted.sort((a, b) => {
//         if (typeof a[column] === 'string' && typeof a[column] === 'string') {
//             let x = a[column] as string
//             let y = b[column] as string
//             if (x.toUpperCase() < y.toUpperCase()) return -1
//             if (x.toUpperCase() > y.toUpperCase()) return 1
//             return 0
//         } else {
//             let x = a[column] as number
//             let y = b[column] as number
//             return x - y;
//         }
//     })
// }

// function sortDataDsce(column: keyof RecordTFA, data: RecordTFA[]) {
//     const sorted = [...data]
//     return sorted.sort((a, b) => {
//         if (typeof a[column] === 'string' && typeof a[column] === 'string') {
//             let x = a[column] as string
...
const toggleTooltip = (event: MouseEvent) => {
    ...
    // console.log('toggleTooltip', target.scrollWidth, target.offsetWidth);
    ...
    // console.log('toggleTooltip', isDisableTooltip.value);
}

const sortAscending = async (index: number, item: string) => {
    console.log('function sortAscending', index, item);
...
const sortDescending = async (index: number, item: string) => {
    console.log('function sortDescending', index, item);
...
const ResetSort = async (index: number, item: string) => {
    console.log('function ResetSort', index, item);
...
const selectAllData = (_bool: boolean) => {
    console.log('function selectAllData', _bool);

    let list: RecordTFA[] = [...recordTfaList.value]
    if (!_bool) {
        const temp = list.filter(data => { return selectList.value.every(i => i.id !== data.ruleId + data.startTime) })
        temp.forEach(data => { selectList.value.push({ id: data.ruleId + data.startTime, data: data }) })
        tableSelectList.value = tableSelectList.value.map(i => { return { id: i.id, selected: true } })
    } else {
        selectList.value = selectList.value.filter((item) => !list.some(i => (i.ruleId + i.startTime) === (item.id)))
        tableSelectList.value = tableSelectList.value.map(i => { return { id: i.id, selected: false } })
    }
}
const selectData = (_bool: any, data: RecordTFA, id: string) => {
    console.log('function selectData', _bool, data, id);
...
const createSelectMatrix = (columns: number): SelectList[] => {
    console.log('function createSelectMatrix');
    ...
    console.log('function createSelectMatrix', newMatrix);

    return newMatrix;
};
// 初始化table的selectionList
const initialSelectMatrix = (columns: number = recordTfaList.value.length): SelectList[] => {
    console.log('function initialSelectMatrix');

    const newMatrix: SelectList[] = [];
    for (let i = 0; i < columns; i++) {
        const item: SelectList = {
            id: recordTfaList.value[i].ruleId + recordTfaList.value[i].startTime,
        }
        newMatrix.push(item)
    }
    newMatrix.forEach(item => item.selected = selectList.value.some(i => i.id === item.id))

    console.log('function initialSelectMatrix', newMatrix);

    return newMatrix;
};

const handleResetAll = () => {
    console.log('function handleResetAll');
...
const selectDropdown = (item: any, type: string, _bool: any) => {
    console.log('function selectDropdown', item, type, _bool);
...
const selectAllDropdown = (type: string) => {
    console.log('function selectAllDropdown');
...
const clearDropdown = (type: string) => {
    console.log('function clearDropdown');
    if (type === t('record.Stream_name')) {
...
const applyDropdownFilter = async () => {
    console.log('function applyDropdownFilter');
    resetCurrentPage()
...
watch(resetTiming, async () => {
    console.log('watch resetTiming');
    handleResetAll()
})

watch(conditionForTfaApi, (newValue, oldValue) => {
    console.log('watch conditionForTfaApi', newValue, oldValue);

    let bool = (newValue.sTime !== oldValue.sTime) || (newValue.eTime !== oldValue.eTime) || (newValue.period !== oldValue.period) || (newValue.filterType !== oldValue.filterType) || (newValue.filterValue !== oldValue.filterValue) || (newValue.channelNameFilter !== oldValue.channelNameFilter) || (newValue.eventNameFilter !== oldValue.eventNameFilter) || (newValue.hideStreamFilter !== oldValue.hideStreamFilter)
...
watch(recordTfaList, () => {
    console.log('watch recordTfaList');

    if (recordTfaList.value.length === 0) return
...
<template>
    ...
    <div v-if="recordTfaList.length !== 0 && isDataLoaded && isPageNoLoaded" class="tableWrapper scrollbar">
        ...
        <table class="tableContent">
            <thead class="tableHeader">
                <tr>
                    ...
                    <th v-for="(label, index) in headerList.slice(0, 4)" :key="label.name">
                        <div
                            :class="{ dropDownMenu: label.name === t('record.Stream_name') || label.name === t('record.Event_name'), nameFilter: label.name === t('record.Stream_name') || label.name === t('record.Event_name') }">

                            <input v-if="label.name === t('record.Stream_name') || label.name === t('record.Event_name')"
                                type="checkbox" :id="label.name">
                            ...
                            <label v-if="label.name === t('record.Stream_name') || label.name === t('record.Event_name')"
                                :for="label.name" class="list__head__title__sort">
                                <img alt="img failed" src="\arrow_drop_down.svg">
                            </label>
                            <button v-else-if="sortDirectionList[index] === 'none'"
                                @click="sortAscending(index, label.apiLabel)"
                                class="list__head__title__sort">
                                <img alt="img failed" src="\nonSort.svg" >
                            </button>
                            <button v-else-if="sortDirectionList[index] === 'asce'"
                                @click="sortDescending(index, label.apiLabel)"
                                class="list__head__title__sort">
                                <img alt="img failed" src="\asceSort.svg" >
                            </button>
                            <button v-else-if="sortDirectionList[index] === 'dsce'"
                                @click="ResetSort(index, label.apiLabel)" 
                                class="list__head__title__sort">
                                <img alt="img failed" src="\dsceSort.svg" >
                            </button>
                    ...
                    <th v-for="(label, index) in headerList.slice(4, 10)" :key="label.name"
                        v-show="vehicleTypeShowed[index].visible">
                        <div>
                            <button v-if="sortDirectionList[index + 4] === 'none'"
                                @click="sortAscending(index + 4, label.apiLabel)" class="list__head__title__sort"><img alt="img failed"
                                    src="\nonSort.svg" ></button>
                            <button v-else-if="sortDirectionList[index + 4] === 'asce'"
                                @click="sortDescending(index + 4, label.apiLabel)" class="list__head__title__sort"><img alt="img failed"
                                    src="\asceSort.svg" ></button>
                            <button v-else-if="sortDirectionList[index + 4] === 'dsce'"
                                @click="ResetSort(index + 4, label.apiLabel)" class="list__head__title__sort"><img alt="img failed"
                    ...
                    <th v-show="isTotalShow">
                        <div>
                            <button v-if="sortDirectionList[10] === 'none'" @click="sortAscending(10, 'Total')"
                                class="list__head__title__sort"><img alt="img failed" src="\nonSort.svg" ></button>
                            <button v-else-if="sortDirectionList[10] === 'asce'" @click="sortDescending(10, 'Total')"
                                class="list__head__title__sort"><img alt="img failed" src="\asceSort.svg" ></button>
                            <button v-else-if="sortDirectionList[10] === 'dsce'" @click="ResetSort(10, 'Total')"
                                class="list__head__title__sort"><img alt="img failed" src="\dsceSort.svg" ></button>
                            <VTooltip :disabled="isDisableTooltip">
                                <span @mouseover="toggleTooltip">{{ $t('record.Total') }}</span>
                                <template #popper>
                                    {{ $t('record.Total') }}
                                </template>
                            </VTooltip>
            ...
            <tbody class="tableBody">
                <tr v-for="(data, index) in recordTfaList" :key="data.ruleId + data.startTime">
                    <th class="selector">
                        <label>
                    ...
                    <td>
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ data.startTime }}</div>
                            <template #popper>
                                {{ data.startTime }}
                            </template>
                        </VTooltip>
                    ...
                    <td v-show="vehicleTypeShowed[0].visible">
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ data.truck }}</div>
                            <template #popper>
                                {{ data.truck }}
                            </template>
                        </VTooltip>
                    </td>
                    <td v-show="vehicleTypeShowed[1].visible">
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ data.bus }}</div>
                            <template #popper>
                                {{ data.bus }}
                            </template>
                        </VTooltip>
                    </td>
                    <td v-show="vehicleTypeShowed[2].visible">
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ data.car }}</div>
                            <template #popper>
                                {{ data.car }}
                            </template>
                        </VTooltip>
                    </td>
                    <td v-show="vehicleTypeShowed[3].visible">
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ data.motorbike }}</div>
                            <template #popper>
                                {{ data.motorbike }}
                            </template>
                        </VTooltip>
                    </td>
                    <td v-show="vehicleTypeShowed[4].visible">
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ data.bike }}</div>
                            <template #popper>
                                {{ data.bike }}
                            </template>
                        </VTooltip>
                    </td>
                    <td v-show="vehicleTypeShowed[5].visible">
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ data.person }}</div>
                            <template #popper>
                                {{ data.person }}
                            </template>
                        </VTooltip>
                    </td>
                    <td v-show="isTotalShow">
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ data.total }}</div>
                            <template #popper>
                                {{ data.total }}
                            </template>
                        </VTooltip>
                ...
                <tr v-for="(i, index) in blankRowShow" :key="index" class="blank">
                </tr>
            </tbody>
            <tfoot class="tableFooter" v-show="isTotalShow">
                <tr>
                    <th colspan="5">{{ $t('record.Total_number_of_vehicles') }}:</th>
                    <td v-show="vehicleTypeShowed[0].visible">
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ recordTfaSumList.totalTruck }}</div>
                            <template #popper>
                                {{ recordTfaSumList.totalTruck }}
                            </template>
                        </VTooltip>
                    </td>
                    <td v-show="vehicleTypeShowed[1].visible">
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ recordTfaSumList.totalBus }}</div>
                            <template #popper>
                                {{ recordTfaSumList.totalBus }}
                            </template>
                        </VTooltip>
                    </td>
                    <td v-show="vehicleTypeShowed[2].visible">
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ recordTfaSumList.totalCar }}</div>
                            <template #popper>
                                {{ recordTfaSumList.totalCar }}
                            </template>
                        </VTooltip>
                    </td>
                    <td v-show="vehicleTypeShowed[3].visible">
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ recordTfaSumList.totalMotorbike }}</div>
                            <template #popper>
                                {{ recordTfaSumList.totalMotorbike }}
                            </template>
                        </VTooltip>
                    </td>
                    <td v-show="vehicleTypeShowed[4].visible">
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ recordTfaSumList.totalBike }}</div>
                            <template #popper>
                                {{ recordTfaSumList.totalBike }}
                            </template>
                        </VTooltip>
                    </td>
                    <td v-show="vehicleTypeShowed[5].visible">
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ recordTfaSumList.totalPerson }}</div>
                            <template #popper>
                                {{ recordTfaSumList.totalPerson }}
                            </template>
                        </VTooltip>
                    </td>
                    <td>
                        <VTooltip :disabled="isDisableTooltip">
                            <div @mouseover="toggleTooltip">{{ recordTfaSumList.totalSum }}</div>
                            <template #popper>
                                {{ recordTfaSumList.totalSum }}
                            </template>
                        </VTooltip>
                    </td>
                </tr>
    ...
    <div v-else-if="recordTfaList.length === 0 && isDropdownFilterUsed && isDataLoaded && isPageNoLoaded">
        <table class="tableContent">
            <thead class="tableHeader">
                <tr>
                    <th class="headerSelector">
                    ...
                    <th v-for="(label, index) in headerList.slice(0, 4)" :key="label.name">
                        <div
                            ...
                            <label v-if="label.name === t('record.Stream_name') || label.name === t('record.Event_name')"
                                :for="label.name" class="list__head__title__sort"><img alt="img failed" src="\arrow_drop_down.svg"
                                    ></label>
                            <button v-else-if="sortDirectionList[index] === 'none'"
    ...
    <div v-else-if="recordTfaList.length === 0 && filterValue && isDataLoaded && isPageNoLoaded" class="tableWrapper noResult">
        <img alt="img failed" src="\img_no_search_result.png"  class="noResult__img">
        <span class="noResult__text">{{ $t('record.No_matching_record_Please_search_again') }}</span>
    </div>

    <div v-else-if="recordTfaList.length === 0 && isDataLoaded && isPageNoLoaded" class="tableWrapper noResult">
        <img alt="img failed" src="\img_record_EmptyState.png"  class="noResult__img">
        <span class="noResult__text">{{ $t('record.You_dont_have_any_record_yet') }}</span>
    </div>
